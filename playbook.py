...
def similarmentions(entity, mentionarray):
    w = len(mentionarray)
    mentiondata = [0 for x in range(w)]
    for i in range(0,w):
        similarityscore = similar(entity,mentionarray.iloc[i])
        mentiondata[i] = similarityscore
    return mentiondata
...
h, q = num_mentions, num_lines
fullentitymentiondata = [[0 for x in range(h)]for y in range(q)]
...
for i in range (0,num_lines):
    ...
    entitymentiondata = similarmentions(entity,entitymentions)
...
for j in range(len(X_test)):
    ...
    accuracy = metrics.accuracy_score(np.array(y_true), np.array(y_pred).round())
    accuracies[j] = accuracy
    print('Accuracy: ', accuracy)
    precision = metrics.precision_score(np.array(y_true), np.array(y_pred).round())
    precisions[j] = precision
    print('Precision: ', precision)
    f1 = metrics.f1_score(np.array(y_true), np.array(y_pred).round())
    f1s[j] = f1
    print('F1 Score: ', f1)
    recall = metrics.recall_score(np.array(y_true), np.array(y_pred).round())
    recalls[j] = recall
    print('Recall: ', recall)
...
#print(similar('GSW', 'Golden State Warriors'))
#x = "Golden State Warriors"
#get an acronym for a word
